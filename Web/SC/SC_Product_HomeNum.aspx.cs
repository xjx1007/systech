...
public partial class Web_SC_SC_Product_HomeNum : BasePage
{
    ...
    private void GetBomDetails()
    {
        ...
        int sd = int.Parse(Request.QueryString["Number"].ToString());
        string SuppNo = Request.QueryString["HouseNo"].ToString();
        string housesql = "select * from KNet_Sys_WareHouse where SuppNo='" + SuppNo + "'";//查询仓库编号
        this.BeginQuery(housesql);
        DataSet Dts_DemoHouse = (DataSet)this.QueryForDataSet();
        DataTable Dtb_DemoHouse = Dts_DemoHouse.Tables[0];
        ...
        if (Dtb_DemoProducts.Rows.Count > 0)
        {
            ...
            for (int i = 0; i < Dtb_DemoProducts.Rows.Count; i++)
            {
                ...
                Sb_BomDetails.Append("<td class=\"ListHeadDetails\">" + Dtb_DemoProducts.Rows[i]["PBC_Name"].ToString() +
                                     ...
                                     "%)</td>");
                Sb_BomDetails.Append("<td class=\"ListHeadDetails\">" +
                                     Base_GetHouseAndNumber1(Dtb_DemoProducts.Rows[i]["XPD_ProductsBarCode"].ToString(), int.Parse(Dtb_DemoProducts.Rows[i]["XPD_Number"].ToString()), HouseNo) +
                                     "</td>");
    ...
    public string Base_GetHouseAndNumber1(string s_ProductsBarCode, int num, string HouseNo)
    {
        ...
        try
        {
            ...
            string s_Sql =
                ...
                s_ProductsBarCode + "' and a.HouseType='0' and a.HouseNo='" + HouseNo + "'  Group by a.HouseNo ";
            ...
            if (Dtb_Table.Rows.Count > 0)
            {
                for (int i = 0; i < Dtb_Table.Rows.Count; i++)
                {
                    ...
                    string sc_Sql =
                        "Select wrkNumber,a.ProductsBarCode from Knet_Procure_OrdersList_Details a join v_OrderRkDetails c on c.KPO_ID=a.ID join Knet_Procure_OrdersList b  on a.OrderNo =b.OrderNo Where  a.OrderNo in(select OrderNo FROM Knet_Procure_OrdersList a  join v_OrderRKWithNoDel b on a.OrderNO=b.V_OrderNo join KNet_Sys_ProcureType c on a.OrderType=c.ProcureTypeValue where rkState!=1 and OrderType='128860698200781250'and a.KPO_Del!=1) and a.ProductsBarCode in (select XPD_FaterBarCode from Xs_Products_Prodocts_Demo where XPD_ProductsBarCode='" + s_ProductsBarCode + "') and SuppNo in(select SuppNo from KNet_Sys_WareHouse where HouseNo='" +
                        Dtb_Table.Rows[i][1].ToString() + "')";
                    ...
                    int count = (int.Parse(Dtb_Table.Rows[i][0].ToString()) + b - c);
                    if (count <= 0)
                    {
