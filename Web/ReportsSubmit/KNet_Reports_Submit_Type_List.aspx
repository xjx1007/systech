<%@ Page Language="C#" AutoEventWireup="true" CodeFile="KNet_Reports_Submit_Type_List.aspx.cs" Inherits="Web_Sales_KNet_Reports_Submit_Type_List" %>
...
<tr>
	...
	<a class="hdrLink" href="KNet_Reports_Submit_Type_List.aspx">部门报表类型设置</a>
        </td>
	<td width=100% nowrap>
		...
		<td class="small" >
			...
			<td>
				...
				<tr>
				<td style="padding-right:0px;padding-left:10px;"><a href="javascript:;"  onclick="PageGo('KNet_Reports_Submit_Type_Add.aspx')"><img src="../../themes/softed/images/btnL3Add.gif" alt="创建 部门报表类型设置..." title="创建 部门报表类型设置..." border=0></a></td>
				...
				<td style="padding-right:10px"><a href="javascript:;" onClick="ShowDiv()" ><img src="../../themes/softed/images/btnL3Search.gif" alt="查找 部门报表类型设置..." title="查找 部门报表类型设置..." border=0></a></td>
				<td style="padding-right:0px;padding-left:10px;"><img src="../../themes/softed/images/tbarImport.gif" alt="*导入 部门报表类型设置" title="*导入 部门报表类型设置" border="0"></td>	
				<td style="padding-right:10px"><img src="../../themes/softed/images/tbarExport.gif" alt="*导出 部门报表类型设置" title="*导出 部门报表类型设置" border="0"></td>
				</tr>
				</table>
			</td>
			</tr>
...
<tr>
 <td>
    <%=base.Base_BindView("KNet_Reports_Submit_Type", "KNet_Reports_Submit_Type_List.aspx", Request.QueryString["WhereID"] == null ? "" : Request.QueryString["WhereID"].ToString())%>
</td>
</tr>
                        ...
                        <table border="0" cellspacing="0" cellpadding="5" width="80%" class="searchUIAdv3 small"
                            ...
                            <tr>
                                <td align="left" width="40%">
                                    <input type="button" name="more" value=" 增加条件 " onclick="fnAddSrch('<%=Base_GetBindSearch("KNet_Reports_Submit_Type")%>','<option value=\'cts\'>包含</option><option value=\'dcts\'>不包含</option><option value=\'is\'>等于</option><option value=\'isn\'>不等于</option><option value=\'bwt\'>开始为</option><option value=\'grt\'>大于</option><option value=\'lst\'>小于</option><option value=\'grteq\'>大于等于</option><option value=\'lsteq\'>小于等于</option>')"
    ...
    <cc1:MyGridView ID="MyGridView1" runat="server" AllowPaging="True" AllowSorting="True" IsShowEmptyTemplate="true"   EmptyDataText="<div align=center><font color=red><br/><br/><B>没有找到相关报表记录</B><br/><br/></font></div>" 
     ...
     <Columns>
        <asp:TemplateField>
         ...
         <asp:TemplateField HeaderText="编码"  SortExpression="KRT_ID" ItemStyle-HorizontalAlign="Left" HeaderStyle-HorizontalAlign="center">
          <ItemTemplate>
               <a href="KNet_Reports_Submit_Type_View.aspx?ID=<%# DataBinder.Eval(Container.DataItem, "KRT_ID") %>"><%# DataBinder.Eval(Container.DataItem, "KRT_Code").ToString()%></a>
          </ItemTemplate>
        </asp:TemplateField>
      <asp:TemplateField HeaderText="名称"  SortExpression="KRT_TypeName" ItemStyle-HorizontalAlign="Left" HeaderStyle-HorizontalAlign="center">
          <ItemTemplate>
               <%# DataBinder.Eval(Container.DataItem, "KRT_TypeName").ToString()%>
          </ItemTemplate>
        </asp:TemplateField>
      <asp:TemplateField HeaderText="类型"  SortExpression="KRT_Type" ItemStyle-HorizontalAlign="Left" HeaderStyle-HorizontalAlign="center">
          <ItemTemplate>
               <%# base.Base_GetBasicCodeName("136",DataBinder.Eval(Container.DataItem, "KRT_Type").ToString())%>
          </ItemTemplate>
        </asp:TemplateField>
        
        
      <asp:TemplateField HeaderText="部门"  SortExpression="KRT_Depart" ItemStyle-HorizontalAlign="Left" HeaderStyle-HorizontalAlign="center">
          <ItemTemplate>
               <%# base.Base_GeDept(DataBinder.Eval(Container.DataItem, "KRT_Depart").ToString())%>
          </ItemTemplate>
        </asp:TemplateField>
      <asp:TemplateField HeaderText="人员"  SortExpression="KRT_Person" ItemStyle-HorizontalAlign="Left" HeaderStyle-HorizontalAlign="center">
          <ItemTemplate>
               <%# base.Base_GetUserName(DataBinder.Eval(Container.DataItem, "KRT_Person").ToString())%>
          </ItemTemplate>
        </asp:TemplateField>
      <asp:TemplateField HeaderText="截止时间"  SortExpression="KRT_TypeTime" ItemStyle-HorizontalAlign="Left" HeaderStyle-HorizontalAlign="center">
          <ItemTemplate>
               <%# DataBinder.Eval(Container.DataItem, "KRT_TypeTime").ToString()%>
          </ItemTemplate>
        </asp:TemplateField>
      <asp:TemplateField HeaderText="创建人"  SortExpression="KRT_Creator" ItemStyle-HorizontalAlign="Left" HeaderStyle-HorizontalAlign="center">
          <ItemTemplate>
               <%# base.Base_GetUserName(DataBinder.Eval(Container.DataItem, "KRT_Creator").ToString())%>
          </ItemTemplate>
        </asp:TemplateField>
         <asp:BoundField  HeaderText="创建时间"  DataField="KRT_CTime"  SortExpression="KRT_CTime" >
        ...
        <asp:TemplateField HeaderText="修改" HeaderStyle-HorizontalAlign="center" >
            <ItemTemplate>  
                <a href="KNet_Reports_Submit_Type_Add.aspx?ID=<%# DataBinder.Eval(Container.DataItem, "KRT_ID") %>"><asp:Image ID="Image1" runat="server"  ImageUrl="../../images/Edit.gif" border=0  ToolTip="修改" /></a>
           </ItemTemplate>
        </asp:TemplateField>
</Columns>
