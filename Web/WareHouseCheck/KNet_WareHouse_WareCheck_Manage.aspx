<%@ Page Language="C#" AutoEventWireup="true" CodeFile="KNet_WareHouse_WareCheck_Manage.aspx.cs" Inherits="Web_Sales_KNet_WareHouse_WareCheck_Manage" %>
	...
	<td width=100% nowrap>
		...
		<td class="small" >
			...
			<td>
				...
				<tr>
				<td style="padding-right:0px;padding-left:10px;"><a href="javascript:;"  onclick="PageGo('KNet_WareHouse_WareCheck_Add.aspx?DType=<%=s_DType %>')"><img src="../../themes/softed/images/btnL3Add.gif" alt="创建 库存盘点..." title="创建 库存盘点..." border=0></a></td>
				...
				<td style="padding-right:10px"><a href="javascript:;" onClick="ShowDiv()" ><img src="../../themes/softed/images/btnL3Search.gif" alt="查找 库存盘点..." title="查找 库存盘点..." border=0></a></td>
				<td style="padding-right:0px;padding-left:10px;"><img src="../../themes/softed/images/tbarImport.gif" alt="*导入 库存盘点" title="*导入 库存盘点" border="0"></td>	
				<td style="padding-right:10px"><img src="../../themes/softed/images/tbarExport.gif" alt="*导出 库存盘点" title="*导出 库存盘点" border="0"></td>
				</tr>
				</table>
			</td>
			</tr>
...
<tr>
 <td>
    <%=base.Base_BindView("KNet_WareHouse_WareCheck", "KNet_WareHouse_WareCheck_List.aspx", Request.QueryString["WhereID"] == null ? "" : Request.QueryString["WhereID"].ToString())%>
</td>
</tr>
                        ...
                        <table border="0" cellspacing="0" cellpadding="5" width="80%" class="searchUIAdv3 small"
                            ...
                            <tr>
                                <td align="left" width="40%">
                                    <input type="button" name="more" value=" 增加条件 " onclick="fnAddSrch('<%=Base_GetBindSearch("KNet_WareHouse_WareCheck")%>','<option value=\'cts\'>包含</option><option value=\'dcts\'>不包含</option><option value=\'is\'>等于</option><option value=\'isn\'>不等于</option><option value=\'bwt\'>开始为</option><option value=\'grt\'>大于</option><option value=\'lst\'>小于</option><option value=\'grteq\'>大于等于</option><option value=\'lsteq\'>小于等于</option>')"
...
<Columns>
        ...
        <asp:TemplateField>
         ...
         <asp:TemplateField HeaderText="盘点编号"  SortExpression="WareCheckNo" ItemStyle-HorizontalAlign="Left" HeaderStyle-HorizontalAlign="center">
          <ItemTemplate>
               <a href="KNet_WareHouse_WareCheck_View.aspx?ID=<%# DataBinder.Eval(Container.DataItem, "WareCheckNo") %>"><%# DataBinder.Eval(Container.DataItem, "WareCheckNo").ToString()%></a>
               </ItemTemplate>
        </asp:TemplateField>
         <asp:BoundField   HeaderText="盘点日期"  DataField="WareCheckDateTime"   SortExpression="WareCheckDateTime"  DataFormatString="{0:yyyy-MM-dd}" HtmlEncode="false">
            <ItemStyle HorizontalAlign="Left" Font-Size="12px"  />
            <HeaderStyle  HorizontalAlign="Left" Font-Size="12px" />
        </asp:BoundField>
        
        <asp:TemplateField HeaderText="盘点仓库"  SortExpression="HouseNo" HeaderStyle-Font-Size="12px"   ItemStyle-HorizontalAlign="Left" HeaderStyle-HorizontalAlign="Left">
          <ItemTemplate>
               <%# base.Base_GetHouseName(DataBinder.Eval(Container.DataItem, "HouseNo").ToString())%>
          </ItemTemplate>
        </asp:TemplateField>
      <asp:TemplateField HeaderText="经手人"  SortExpression="WareCheckStaffNo" HeaderStyle-Font-Size="12px"   ItemStyle-HorizontalAlign="Left" HeaderStyle-HorizontalAlign="Left">
          <ItemTemplate>
               <%# base.Base_GetUserName(DataBinder.Eval(Container.DataItem, "WareCheckStaffNo").ToString())%>
          </ItemTemplate>
        </asp:TemplateField>
        
         <asp:TemplateField HeaderText="审核人"  SortExpression="WareCheckCheckStaffNo" HeaderStyle-Font-Size="12px"    ItemStyle-HorizontalAlign="Left" HeaderStyle-HorizontalAlign="Left">
          <ItemTemplate>
               <%# base.Base_GetUserName(DataBinder.Eval(Container.DataItem, "WareCheckCheckStaffNo").ToString())%>
          </ItemTemplate>
        </asp:TemplateField>
        <asp:TemplateField HeaderText="修改" HeaderStyle-HorizontalAlign="center" >
            <ItemTemplate>
                <a href="KNet_WareHouse_WareCheck_Add.aspx?ID=<%# DataBinder.Eval(Container.DataItem, "WareCheckNo") %>"><asp:Image ID="Image1" runat="server"  ImageUrl="../../images/Edit.gif" border=0  ToolTip="修改" /></a>
           </ItemTemplate>
        </asp:TemplateField>
      <asp:TemplateField HeaderText="审核"  SortExpression="WareHouseCheckYN" HeaderStyle-Font-Size="12px"  ItemStyle-Width="30px"   ItemStyle-HorizontalAlign="center" HeaderStyle-HorizontalAlign="center">
          <ItemTemplate>
