...
public partial class Web_ProductSubmitted_Knet_OQCSubmitted_Product_List :BasePage
{
    ...
    protected void DataShows()
    {
        ...
        string SqlWhere = " 1=1 and KSP_Type=2  ";
    ...
    /// </summary>
    /// <returns></returns>
    public string Base_GetDbNumber(string s_ID)
    {
        String s_Name = "";
        this.BeginQuery("select  KPD_Number,KPD_Code from Knet_Submitted_Product_Details where KPD_SID = '" + s_ID + "'");
        this.QueryForDataTable();
        DataTable Dtb_Re = Dtb_Result;
        if (Dtb_Re.Rows.Count > 0)
        {
            for (int i = 0; i < Dtb_Re.Rows.Count; i++)
            {
                this.BeginQuery("select isnull(sum(AllocateAmount),0 ) from KNet_WareHouse_FuAllocateList a join KNet_WareHouse_FuAllocateList_Details b on a.AllocateNo=b.AllocateNo where a.KSP_SID='" + s_ID + "' and b.ProductsBarCode='"+ Dtb_Re.Rows[i]["KPD_Number"].ToString() + "'");
                this.QueryForDataTable();
                DataTable Dtb_Re1 = Dtb_Result;
                s_Name += Dtb_Re1.Rows[0][0].ToString();
                s_Name += "<br/>";
            }
