...
public partial class Web_Report_Bill_Report_Tax_Rate : BasePage
{
    ...
    protected void Button1_OnServerClick(object sender, EventArgs e)
    {
        try
        {
            if (New_TaxRate.Text==""|| Old_TaxRate.Text=="")
            {
                Response.Write("<script language=javascript>alert('输入不能为空')</script>");
            }
            else
            {
                string insert_sql = "insert into KNet_Tax_Rate (Old_TaxRate,New_TaxRate,CTime) VALUES('" + Old_TaxRate.Text +
                               "','" + New_TaxRate.Text + "','" + DateTime.Now + "') "; //插入一条洗的
                ...
                string update_Handprice =
                    "update Knet_Procure_SuppliersPrice set KPP_PCPrice=CAST((KPP_PCPrice/" + oldTaxRate + ")*" + newTaxRate +
                    " as decimal(18,2)),HandPrice=cast((HandPrice/" + oldTaxRate + ")*" + newTaxRate +
                    " as decimal(18,2)),ProcureUnitPrice=cast((ProcureUnitPrice/" + oldTaxRate + ")*" + newTaxRate +
                    " as decimal(18,4))";
