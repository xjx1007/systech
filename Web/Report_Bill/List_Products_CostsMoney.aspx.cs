...
public partial class Web_Report_Bill_List_Products_CostsMoney : BasePage
{
        ...
        if (!IsPostBack)
        {
            ...
            if (Dtb_Table.Rows.Count > 0)
            {
                for (int i = 0; i < Dtb_Table.Rows.Count; i++)
                {
                    ...
                    s_Details += "<td  class='thstyleLeftDetails'align=center noWrap>" +
                               Dtb_Table.Rows[i]["SER_ScHandFaxMoney"].ToString() + "</td>\n";
                    s_Details += "<td  class='thstyleLeftDetails'align=center noWrap>" +
                                 Dtb_Table.Rows[i]["SER_MaterFaxMoney"].ToString() + "</td>\n";

                    s_Details += "<td  class='thstyleLeftDetails' align=right  noWrap>" +
                                 Dtb_Table.Rows[i]["SER_DirectMaterMoney"].ToString() + "</td>\n";
                    s_Details += "<td  class='thstyleLeftDetails' align=right  noWrap>" +
                                Dtb_Table.Rows[i]["SER_CountMakeMoney"].ToString() + "</td>\n";
                    s_Details += "<td  class='thstyleLeftDetails' align=right  noWrap>" +
                               Dtb_Table.Rows[i]["SER_ScManMoney"].ToString() + "</td>\n";
                    s_Details += "<td  class='thstyleLeftDetails' align=right  noWrap>" +
                              Convert.ToDecimal(Dtb_Table.Rows[i]["SER_ScHandFaxMoney"])+ Convert.ToDecimal(Dtb_Table.Rows[i]["SER_MaterFaxMoney"])+ Convert.ToDecimal(Dtb_Table.Rows[i]["SER_DirectMaterMoney"])+ Convert.ToDecimal(Dtb_Table.Rows[i]["SER_CountMakeMoney"])+ Convert.ToDecimal(Dtb_Table.Rows[i]["SER_ScManMoney"]) + "</td>\n";
                    s_Details += " </tr>";
                ...
                s_Head +=
                    "<tr>\n<th colspan=\"12\" class=\"MaterTitle\" style='height:14.25pt'>杭州士腾科技有限公司<br/>生产入库成本费用</th></tr>\n";
                s_Head +=
                ...
                s_Head += "<th class=\"thstyle\" align=center>加工费（不含税）</th>\n";
                s_Head += "<th class=\"thstyle\" align=center>材料费（不含税）</th>\n";
                s_Head += "<th class=\"thstyle\" align=center>其他材料费</th>\n";
                s_Head += "<th class=\"thstyle\" align=center>制造费</th>\n";
                s_Head += "<th class=\"thstyle\"  align=center >人工费用</th>\n";
                s_Head += "<th class=\"thstyle\"  align=center >产品成本</th>\n";
    ...
    protected void Btn_Save_OnClick(object sender, EventArgs e)
    {
        ...
        Excel.HtmlToExcel(this.Lbl_Details.Text, "生产入库人工费用");
    }
}