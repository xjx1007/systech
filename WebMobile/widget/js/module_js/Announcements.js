set_input_msg();
function AnnouncementObj(name){			//构造函数伪装，把Obj实例伪装成Module实例，来继承Module的属性
	Module.call(this,name); 
	for(var attr in Module.prototype){
	      AnnouncementObj.prototype[attr] = Module.prototype[attr];
	};
...
AnnouncementObj.prototype.GetListFromServer=function(){  //获取列表(实例方法)
	this.offset = getstorage("currPage");     // 页码
	var searchtext = getstorage("searchtext");  //搜索条件
	if(!searchtext) searchtext = '';
	var viewsel = getstorage("announcement_viewId");   //视图id
...
AnnouncementObj.prototype.ShowList = function(data){ //显示列表(实例方法)
	//alert(data.lastpg+"result_count:"+data.result_count+"next_offset:"+data.next_offset);
	var forumObj = '';
	...
	if ((listObj !== undefined) && (listObj.entry_list !== undefined)) {
		...
		for (i = 0; i < listObj.entry_list.length; i++) {
			if (listObj.entry_list[i] !== undefined) {
				...
				if(i%2==0)
				whitebg = "";
				forumObj+='<div ontouchstart="zy_touch(\'list-act\')" onclick="openAnnouncementDetail('+theBill.id+',\'\');" class="ubb ub b-gra t-bla umh4 lis3 '+whitebg+'">';
					forumObj+='<div class="avatar"><img src=\"'+theBill.userPhoto+'\"/></div>';
					forumObj+='<div class="ub-f1 ub ub-ver lis3">';	
						...
						forumObj+='<div class="ub ub-ac t-gra ulev-2">';
							...
							forumObj+='<div class="ub-f1"><i class="icon-calendar t-blu"></i>&nbsp;'+theBill.name_value_list.startDate.value+' - '+theBill.name_value_list.endDate.value+'</div>';
		...
		if(listObj.lastpg > 0){
			...
			uexWindow.evaluatePopoverScript('AnnouncementsListPage','pop_nav',"setPageSel()");
		}
	}
}

AnnouncementObj.prototype.setDetailsInfo=function(){  //获取详细(实例方法)
	this.CurrentAnnouncementId = getstorage("CurrentAnnouncementId");  //id
...
AnnouncementObj.prototype.ShowDetail = function(data){ //显示 详细(实例方法)
	var forumObj = '';
	...
	if(data.isedit == 'yes'){
		//uexWindow.evaluateScript('AnnouncementDetailsPage', '0', "showEditButton()");
	}
	...
	if ((detailObj !== undefined) && (detailObj.entry_list !== undefined)) {
		...
		for (var i = 0; i < listlen; i++) {
			...
			for (var j = 0; j < fieldlen; j++) {
				...
				forumObj += '<div class="ub-f1 detail_nr">' + field[j].value + '</div>';
				forumObj += '</div>';
			}
			forumObj += '</div>';
		}
...
AnnouncementObj.prototype.setCreatePageBind=function(){  //创建(实例方法)
...
AnnouncementObj.prototype.setEditPageBind=function(){  //编辑(实例方法)
	this.theWindow = 'EditPage';
	this.CurrentAnnouncementId = getstorage("CurrentAnnouncementId");
	this.action = 'get_edit_field';
	this.rest_data = '[{"session":"' + this.CurrentUserId +'",'+
		'"module_name":"'+this.name+'",'+
		'"action":"'+this.action+'",'+
		'"module_id":"' + this.CurrentAnnouncementId + '"}]';
	this.RequestServer();
}; 

AnnouncementObj.prototype.ShowEditPage = function(data){ //显示编辑表单(实例方法)
	...
	if ((createFields !== undefined) && (createFields.relationship_list !== undefined)) {
		...
		for (var i = 0; i < acclistlen; i++) {
			...
			for (var j = 0; j < fieldlen; j++) {
				...
				if (field[j].name == "announcementname") {
					field[j].value = "公告标题";
					field[j].mandatory = 1;
				}
				if (field[j].name == "description" || field[j].name == "announcement981") {
					field[j].mandatory = 1;
				}
				if (field[j].name == "isgd" ) {
					field[j].fieldtype = "switch";
				...
				forumObj += '<div class="ub ub-f1 ubb ulab ub-ac uinn3 b-gra">';
					forumObj += '<div class="detail_bt">'+ em +field[j].value+':</div>';
					...
					forumObj += '</select>';
					forumObj += '</div>';
				}else if (field[j].name == "announcement981") {
						forumObj += '<input id=\"' + module_name +field[j].name + '\" name=\"' + field[j].name + '\" readonly="" type="text" style="display:none" type="text" class=\"' + module_name + 'Box\">';
						forumObj += '<div class="ub t-bla ub-f1 ulab uinn">';  
							forumObj += '<div class="ub-f1 uinn" id="recevier_name"></div>';
							forumObj += '<div class="uinn ulim uc-a1 t-wh tx_bold" ontouchstart="zy_touch(\'btn-act\')" onclick="openSelectUserPage();"  style="left:.2em;height:1.1em;width:1.1em;background-color:#666666">＋';
							forumObj += '</div>'; 
						forumObj += '</div>';
		...
		setstorage('theMode','editAnnouncements');
	}
}

AnnouncementObj.prototype.ShowCreatePage = function(data){ //显示创建表单(实例方法)
	...
	if ((createFields !== undefined) && (createFields.relationship_list !== undefined)) {
			...
			for (var j = 0; j < fieldlen; j++) {
				...
				forumObj += '<div class="ub ub-f1 ubb ulab ub-ac uinn3 b-gra">';
					...
					if (field[j].fieldtype == "accountid") {
						...
						forumObj += '<div class="ub t-bla ub-f1 ulab uinn" >';  
							forumObj += '<div class="ub-f1 uinn" id="recevier_name"></div>';
							forumObj += '<div class="umw2 ub ub-pc uc-r ub-ac uc-r1" ontouchstart="zy_touch(\'btn-act\')" onclick="openSelectUserPage();">';
							forumObj += '<div class="ub-img umw1 umh1 res8">';
							forumObj += '</div></div>'; 
						forumObj += '</div>';
					}else if (field[j].fieldtype == "opts") {
						var optsarr = accopts[field[j].name].value;
		...
		setHtml("createform", forumObj);
		//$$("buttondiv").style.display='block';	
		setstorage('theMode','addAnnouncements');
	}	
}

AnnouncementObj.prototype.ShowResultToClient=function(method,data){  //显示结果(实例方法)
	switch(this.action){
		case 'get_entry_list':
			this.ShowList(data);
			break;
...
AnnouncementObj.prototype.SaveReturn=function(data){  //完成保存(实例方法)
	if (data.result === "SUCCESS") {
		uexWindow.toast(0, 5, "操作成功", 2000);
		openAnnouncementDetail(data.record,"modules/Announcements/");
		uexWindow.evaluateScript('AnnouncementsListPage', '0', "searchList()");
...
AnnouncementObj.prototype.SaveNew=function(){  //保存(实例方法)
    ...
    if (validInput) {
		...
		this.rest_data = '[{"session":"' + this.CurrentUserId +
			...
			'","link_name_to_fields_array":"","record":"' +this.CurrentAnnouncementId +'"}]';
		this.RequestServer();
	}
}; 

var receiverid = [];
function openAnnouncementDetail(id,res){  //打开详细页面
	setstorage('CurrentAnnouncementId', id);
	openwin('AnnouncementDetailsPage',res);
}
