 using System;
using System.Data;
using System.Text;
using System.Collections.Generic;
using KNet.DBUtility;
using KNet.Model;
namespace KNet.BLL
{
/// <summary>
/// Products_Replace_List
/// </summary>
   public partial class Products_Replace_List
   {
   private readonly KNet.DAL.Products_Replace_List dal=new KNet.DAL.Products_Replace_List();
   public Products_Replace_List()
   {}
  	/// <summary>
 /// 是否存在该记录
 /// </summary>
public bool Exists(string PRL_ID)
{
	return dal.Exists(PRL_ID);
}
  	/// <summary>
 /// 增加一条数据
 /// </summary>
public bool Add(KNet.Model.Products_Replace_List model)
{
	return dal.Add( model);
}
  	/// <summary>
 /// 修改一条数据
 /// </summary>
public bool Update(KNet.Model.Products_Replace_List model)
{
	return dal.Update( model);
}
  	/// <summary>
 /// 删除一条数据
 /// </summary>
public bool Delete(string PRL_ID)
{
	return dal.Delete(PRL_ID);
}
  	/// <summary>
 /// 删除数据
 /// </summary>
public bool UpdateDel(KNet.Model.Products_Replace_List model)
{
	return dal.UpdateDel(model);
}
  	/// <summary>
 /// 批量删除一条数据
 /// </summary>
public bool DeleteList(string PRL_ID)
{
	return dal.DeleteList(PRL_ID);
}
  	/// <summary>
 ///删除父节点数据
 /// </summary>
public bool DeleteByFID(string PRL_ID)
{
	return dal.DeleteByFID(PRL_ID);
}
  	/// <summary>
 ///获取类
 /// </summary>
public KNet.Model.Products_Replace_List GetModel(string PRL_ID)
{
	return dal.GetModel(PRL_ID);
}
  	/// <summary>
 ///获得数据列表
 /// </summary>
public DataSet GetList(string strWhere)
{
	return dal.GetList(strWhere);
}
}
}
