 using System;
using System.Data;
using System.Text;
using System.Collections.Generic;
using KNet.DBUtility;
using KNet.Model;
namespace KNet.BLL
{
/// <summary>
/// Zl_Produce_Problem_Details
/// </summary>
   public partial class Zl_Produce_Problem_Details
   {
   private readonly KNet.DAL.Zl_Produce_Problem_Details dal=new KNet.DAL.Zl_Produce_Problem_Details();
   public Zl_Produce_Problem_Details()
   {}
  	/// <summary>
 /// 是否存在该记录
 /// </summary>
public bool Exists(string ZPPD_ID)
{
	return dal.Exists(ZPPD_ID);
}
  	/// <summary>
 /// 增加一条数据
 /// </summary>
public bool Add(KNet.Model.Zl_Produce_Problem_Details model)
{
	return dal.Add( model);
}
  	/// <summary>
 /// 修改一条数据
 /// </summary>
public bool Update(KNet.Model.Zl_Produce_Problem_Details model)
{
	return dal.Update( model);
}
  	/// <summary>
 /// 删除一条数据
 /// </summary>
public bool Delete(string ZPPD_ID)
{
	return dal.Delete(ZPPD_ID);
}
  	/// <summary>
 /// 删除数据
 /// </summary>
public bool UpdateDel(KNet.Model.Zl_Produce_Problem_Details model)
{
	return dal.UpdateDel(model);
}
  	/// <summary>
 /// 批量删除一条数据
 /// </summary>
public bool DeleteList(string ZPPD_ID)
{
	return dal.DeleteList(ZPPD_ID);
}
  	/// <summary>
 ///删除父节点数据
 /// </summary>
public bool DeleteByFID(string ZPPD_ID)
{
	return dal.DeleteByFID(ZPPD_ID);
}
  	/// <summary>
 ///获取类
 /// </summary>
public KNet.Model.Zl_Produce_Problem_Details GetModel(string ZPPD_ID)
{
	return dal.GetModel(ZPPD_ID);
}
  	/// <summary>
 ///获得数据列表
 /// </summary>
public DataSet GetList(string strWhere)
{
	return dal.GetList(strWhere);
}
}
}
