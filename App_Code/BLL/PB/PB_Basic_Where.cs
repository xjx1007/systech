 using System;
using System.Data;
using System.Text;
using System.Collections.Generic;
using KNet.DBUtility;
using KNet.Model;
namespace KNet.BLL
{
/// <summary>
/// PB_Basic_Where
/// </summary>
   public partial class PB_Basic_Where
   {
   private readonly KNet.DAL.PB_Basic_Where dal=new KNet.DAL.PB_Basic_Where();
   public PB_Basic_Where()
   {}
  	/// <summary>
 /// 是否存在该记录
 /// </summary>
public bool Exists(string PBW_ID)
{
	return dal.Exists(PBW_ID);
}
  	/// <summary>
 /// 增加一条数据
 /// </summary>
public bool Add(KNet.Model.PB_Basic_Where model)
{
	return dal.Add( model);
}
  	/// <summary>
 /// 修改一条数据
 /// </summary>
public bool Update(KNet.Model.PB_Basic_Where model)
{
	return dal.Update( model);
}
  	/// <summary>
 /// 删除一条数据
 /// </summary>
public bool Delete(string PBW_ID)
{
	return dal.Delete(PBW_ID);
}
  	/// <summary>
 /// 删除数据
 /// </summary>
public bool UpdateDel(KNet.Model.PB_Basic_Where model)
{
	return dal.UpdateDel(model);
}
  	/// <summary>
 /// 批量删除一条数据
 /// </summary>
public bool DeleteList(string PBW_ID)
{
	return dal.DeleteList(PBW_ID);
}
  	/// <summary>
 ///删除父节点数据
 /// </summary>
public KNet.Model.PB_Basic_Where GetModel(string PBW_ID)
{
	return dal.GetModel(PBW_ID);
}
  	/// <summary>
 ///获得数据列表
 /// </summary>
public DataSet GetList(string strWhere)
{
	return dal.GetList(strWhere);
}
}
}
